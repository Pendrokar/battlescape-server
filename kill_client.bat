...
if not "%~1"=="" (
    ...
    if errorlevel 1 (
        ...
        echo Example: %~nx0 1234
        goto :usage
    )
    set "TARGET=%~1"
    set "SOURCE=argument"
) else if exist "%IB_CLIENT_PID_FILE%" (
    set /p TARGET=<"%IB_CLIENT_PID_FILE%"
    set "SOURCE=pid file"
)
...
if errorlevel 1 (
    ...
    echo Saved client PID %TARGET% is not a running IB process. Searching by command line...
    goto :fallback
)

if exist "%IB_CLIENT_PID_FILE%" (
    set /p FILEPID=<"%IB_CLIENT_PID_FILE%"
    if "!FILEPID!"=="%TARGET%" del "%IB_CLIENT_PID_FILE%" >nul 2>&1
)
exit /b 0
...
if errorlevel 2 (
    if exist "%IB_KILL_MARKER%" del "%IB_KILL_MARKER%" >nul 2>&1
    if exist "%IB_CLIENT_PID_FILE%" del "%IB_CLIENT_PID_FILE%" >nul 2>&1
    echo No Infinity Battlescape client process is running.
    exit /b 0
)

echo Stopped IB client process^(es^):
type "%IB_KILL_MARKER%"
if exist "%IB_CLIENT_PID_FILE%" del "%IB_CLIENT_PID_FILE%" >nul 2>&1
if exist "%IB_KILL_MARKER%" del "%IB_KILL_MARKER%" >nul 2>&1
exit /b 0
